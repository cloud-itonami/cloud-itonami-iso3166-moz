(ns marketentry.governor-contract-test
  ...
  This is the MOZ actor's SIX-check governor (see `marketentry.governor`
  docstring for why it is six, not the AGO reference's seven): there is
  no `nif-unverified`/tax-ID-decree analog test here because there is
  no such check -- the dossier does not ground a distinct NUIT-
  establishing-decree separate from ordinary Cadastro Único
  registration. `cadastro-unico-missing` is the MOZ analog of AGO's
  `ao-entity-missing`/MLI's `rccm-entity-missing`/GNB's `commercial-
  registration-missing` flagship check, but is itself a genuine
  TWO-condition test (missing registration OR active debarment),
  reflecting the Regulamento's own two-registry (eligible + Impedidos)
  design."
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Maputo Fabril Lda"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Maputo Fabril Lda" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest cadastro-unico-missing-is-held-and-unoverridable
  (testing "missing Cadastro Único registration -> HARD hold (MOZ flagship check, missing-registration branch)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:cadastro-unico-missing} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest cadastro-unico-debarred-is-held-and-unoverridable
  (testing "registered but on UFSA's own Impedidos (debarred) list -> HARD hold (MOZ flagship check, debarment branch)"
    (let [[db actor] (fresh)
...
(deftest engagement-not-requiring-cadastro-unico-still-commits-end-to-end
  (testing "eng-5 does NOT itself require Cadastro Único registration -- the conditional branch of the flagship check still commits clean"
    (let [[db actor] (fresh)
