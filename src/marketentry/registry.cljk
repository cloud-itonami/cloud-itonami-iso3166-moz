...
(defn cadastro-unico-satisfied?
  "Does `engagement` independently satisfy the Cadastro Único gate --
  i.e. either it does not itself require Cadastro Único registration,
  or it has valid registration AND is not on UFSA's own Cadastro Único
  ...Impedidos (debarred) list? Pure re-derivation of the same test
  `marketentry.governor`'s flagship check applies, exposed here for
  reuse outside the governor (e.g. by `marketentry.marketentryllm`'s
  own confidence estimate)."
  [{:keys [requires-cadastro-unico? has-cadastro-unico? on-impedidos-list?]}]
  (and (or (not requires-cadastro-unico?) (true? has-cadastro-unico?))
       (not (true? on-impedidos-list?))))
