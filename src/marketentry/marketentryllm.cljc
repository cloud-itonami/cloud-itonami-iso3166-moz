(ns marketentry.marketentryllm
  ...
  the MOZ public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-cadastro-unico?=" (:has-cadastro-unico? e)
                        " on-impedidos-list?=" (:on-impedidos-list? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-cadastro-unico? e))
                              (and (:has-cadastro-unico? e)
                                   (not (:on-impedidos-list? e)))))
                   0.9 0.3)}))
